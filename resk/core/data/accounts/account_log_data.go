...
type AccountLog struct {
	...
	ChangeType      constant.ChangeType `db:"change_type"`          //流水交易类型，0 创建账户，>0 为收入类型，<0 为支出类型，自定义
	ChangeFlag      constant.ChangeFlag `db:"change_flag"`          //交易变化标识：-1 出账 1为进账，枚举
	Status          int                 `db:"status"`               //交易状态：
	Decs            string              `db:"decs"`                 //交易描述
	CreatedAt       time.Time           `db:"created_at,omitempty"` //创建时间
}

func (po *AccountLog) FromTransferDTO(dto *service.AccountTransferDTO) {
